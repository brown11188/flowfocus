...
export async function GET() {
  ...
  const items = await db.select().from(meetingNotes).where(eq(meetingNotes.userId, session.user.id)).orderBy(desc(meetingNotes.meetingDate));
...
export async function POST(req: NextRequest) {
  ...
  const [item] = await db.insert(meetingNotes).values({
    title: String(body.title ?? "").trim(),
    content: body.rawNotes ?? "",
