...
import { db } from "@/db";
import { actionedEmails, emailDigests, microsoftConnections } from "@/db/schema";
...
export async function GET(req: NextRequest) {
  ...
  const actionedRecords = await db
    .select({ microsoftEmailId: actionedEmails.microsoftEmailId })
    .from(actionedEmails)
