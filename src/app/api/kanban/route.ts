...
export async function GET(req: NextRequest) {
  ...
  let columns = await db.query.kanbanColumns.findMany({
  ...
  if (columns.length === 0) {
    ...
    columns = await db.query.kanbanColumns.findMany({
      where: (c, { eq }) => eq(c.projectId, projectId),
      orderBy: (c, { asc }) => [asc(c.sortOrder)],
    });
  }
...
export async function POST(req: NextRequest) {
  ...
  const nextOrder = (r?.maxOrder ?? 0) + 1;

  const [column] = await db
    .insert(kanbanColumns)
    .values({ name: name.trim(), color: color ?? "#6366f1", projectId, sortOrder: nextOrder, isDefault: false })
    .returning();
