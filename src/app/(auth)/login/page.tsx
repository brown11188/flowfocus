...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res?.error) {
      ...
      // Use bare path — Next.js router.push() automatically applies basePath
      router.push("/dashboard");
      router.refresh();
    }
  };

  // For OAuth providers, build a full absolute callbackUrl so NextAuth can
  // redirect back to the correct subpath after OAuth completes.
  ...
  const handleMicrosoft = () => {
    // Redirect to Microsoft Hub after OAuth so the hub detects ?microsoft_connected=true
    const origin = typeof window !== "undefined" ? window.location.origin : "";
    const callbackUrl = `${origin}${basePath}/microsoft?microsoft_connected=true`;
    ...
    <div className="w-full max-w-md">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8 border border-gray-100 dark:border-gray-800">
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs text-gray-400 dark:text-gray-500"><span className="px-2 bg-white dark:bg-gray-900">or continue with email</span></div>
        </div>
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          ...
          <Link href="/register" className="text-violet-600 hover:text-violet-700 font-medium">Sign up</Link>
        </p>
      </div>
    </div>
  );
